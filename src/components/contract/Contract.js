	...
	return (
		<div className="w-full h-screen gap-2 flex justify-between items-center p-4">
			<div
				...
				className={`w-1/6 bg-white h-full overflow-auto rounded-md drop-shadow-xl p-4 flex flex-col ${
					contractViews === false ? "w-1/6" : "w-[0px] hidden"
				}`}
			>
				{/* <div className='mt-4'>
                    <label>Гэрээ</label>
					...
					{contractViews === false && (
						<div className="w-full gap-2">
							{page ? (
								...
								<Pdf targetRef={ref2} filename="гэрээ2.pdf" options={options} x={0} y={0} scale={1}>
									{({ toPdf }) => (
										<button
											className="w-full flex bg-red-600 border mt-2 border-transparent rounded-md shadow-sm py-3 px-4 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-red-500"
											onClick={toPdf}
			...
			{contractViews ? (
				<div
					style={{ transition: ".4s all ease" }}
					className={` bg-white h-full drop-shadow-xl flex overflow-auto flex-wrap justify-start items-start ${
						contractViews ? "w-full" : "w-5/6"
					}`}
				>
					...
					<div className="w-full  bg-white h-full drop-shadow-xl flex overflow-auto flex-wrap justify-start items-start">
						{filteredContractList?.length !== 0 &&
							filteredContractList?.map((item, index) => (
								<div
									key={index}
									className="flex border h-48 w-48 rounded-md p-2  m-2 flex-col items-center justify-center"
								>
				...
				<div
					style={{ transition: ".4s all ease" }}
					className="w-5/6 bg-white h-full rounded-md drop-shadow-xl flex overflow-x-scroll justify-center items-start gap-4"
				>
						...
						<div ref={ref} className="w-[816px] bg-white h-[1104px] drop-shadow-2xl mx-auto py-[37px] px-[37px] ">
							...
							<div className="w-full bg-white flex flex-row gap-2 items-start justify-between mt-4">
								<p className="text-[12px]">
									...
									Гүйцэтгэгч гэх)-ийг төлөөлж менежер С. Бүжинханд нар (цаашид хамтад нь Талууд гэх) Иргэний хуулийн
