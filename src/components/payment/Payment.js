		...
		<div className="w-full md:h-screen  gap-2 flex md:flex-row flex-col md:justify-between  md:items-center item-start p-4">
			<div className="md:w-2/6 w-full bg-white md:h-screen  overflow-auto rounded-md drop-shadow-xl p-4 flex flex-col">
				<div className="mt-4">
					...
					<div className="w-full flex h-12 justify-center items-center gap-2">
						...
						/>
						<button onClick={() => addToExplenation()} className="px-4 py-2 mt-2 rounded-md bg-green-500">
							+
						</button>
					</div>
					<label>Тайлбар</label>
					...
					<Pdf targetRef={ref} filename={`${companyName}_үнийн_санал.pdf`} options={options} x={0} y={0} scale={0.9}>
						{({ toPdf }) => (
							<button
			...
			<div className="'md:w-4/6 w-full bg-white  h-auto rounded-md drop-shadow-xl flex justify-center items-center">
				<div className="absolute top-2 left-2">
					<button onClick={() => navigate("/")} className="border p-2 rounded-md hover:underline border-collapse">
						Буцах
					</button>
				</div>
				...
				<div ref={ref} className="w-[595px] bg-white min-h-[842px] drop-shadow-2xl mx-auto pb-[20px] " wrap={false}>
					...
					<div className="w-full h-full px-[20px] -mt-20">
						<div className="w-full bg-white flex justify-between items-start mt-4 flex-col justify-center mt-2">
							<h2 className="w-full text-center font-bold">{option ? option : "Вэб сайт"} хийх</h2>
							<h1 className="w-full text-[14px] text-center uppercase font-bold">Үнийн санал</h1>
						</div>
						<div className="w-full bg-white flex justify-between items-start mt-4">
							<p className="w-full text-[12px]">Захиалагч байгууллага танаа: {companyName}</p>
										...
										List?.map((item, index) => (
											<tr key={index}>
												<td className="border border-black px-2 border-collapse">
													<p className="text-[12px] text-center">{item.typeName}</p>
													<p className="text-[12px] text-center">{item.tool ? `/${item.tool}/` : ""}</p>
												</td>
												<td className="border border-black border-collapse">
													<p className="text-[12px] text-center">{numberWithCommas(item.Price)}₮</p>
												</td>
												{List.length && List.filter((item) => item.discount).length > 0 && (
													<td className="border border-black border-collapse">
														<p className="text-[12px] text-center">{numberWithCommas(item.discount)}</p>
													</td>
												)}
                        ...
                        <p className="text-[11px] text-center">
                          {numberWithCommas(item.discount)}
										...
										<tr>
											<td className="border border-black border-collapse">
												<p className="text-[12px] text-center">Танилцуулга сайт/Wordpress/</p>
											</td>
											<td className="border border-black border-collapse">
											...
											<td className="border border-black border-collapse">
												<div className="text-[12px] text-center flex flex-col items-start">
													<p className="text-left">- Өгөгдсөн хар зургийн дагуу вэб сайтын дизайныг зурна</p>
													...
													<p className="text-left">
														- Гар утас, таблет гэх мэт бүх төрлийн төхөөрөмжүүд дээр таарч ажиллах
													</p>
